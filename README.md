...
Проект про аренду квартир в Москве. Собирает объявления с Avito, складывает их в Postgres и ClickHouse, раз в сутки переобучает модель CatBoost и отдаёт предсказание месячной аренды через HTTP API. Всё это крутится в локальном Kubernetes и ставится одним Helm-чартом.
...
Для запуска достаточно заполнить один файл — корневой `.env`. Если его нет, `make up` создаст его из `.env.example`, и проект поднимется с настройками по умолчанию. Чтобы парсер ходил через свой ключ spfa.pro и прокси, впишите их туда же:

```bash
AVITO_COOKIES_API_KEY=ваш_ключ
AVITO_PROXY_STRING=login:password@host:port
```

Ни `values.yaml`, ни `config.toml` парсера править не нужно: `make up` сам передаёт значения из `.env` в Kubernetes.
...
4. ставит или обновляет Helm-релиз. Пароли, логины, настройки парсера и обучения берутся из `.env`, путь к репозиторию определяется сам;
...
Все сервисы читают один общий `.env` из корня репозитория, включая парсер Avito. В кластер эти значения тоже попадают из него (см. шаг 4). Подробнее о переменных - в разделе [Конфигурация](#конфигурация).
...
`make up` сам собирает эти значения из `.env` во временный values-файл: путь к репозиторию, логины и пароли Postgres, ClickHouse, MinIO, Airflow и Grafana, `TRAIN_MAX_AGE_DAYS`, `COLLECTOR_INTERVAL_SEC` и все `AVITO_*`. Пустые переменные пропускаются, и для них берётся значение по умолчанию из `values.yaml`. Вручную минимальный вариант выглядит так:
...
Один `.env` в корне на все сервисы. `collector`, `ml` и `inference` загружают его через `python-dotenv`. Уже заданные переменные окружения имеют приоритет над файлом. В Kubernetes цепочка такая: `.env` → `make up` → values Helm-релиза → ConfigMap `airflow-env` и переменные подов. Поды задач Airflow получают их через `envFrom`. Меняете что-то в `.env` — запускаете `make up` ещё раз.

Пароли баз и MinIO применяются только при первой установке, пока диски пустые: Postgres, ClickHouse и MinIO запоминают учётки в своих данных. Чтобы сменить их позже, нужен `make destroy` и чистый `make up`.
...
Эти переменные достаточно один раз вписать в корневой `.env`. Локально их читает `load_config.py`, а в Kubernetes их передаёт `make up`: они попадают в секцию `collector:` релиза и дальше в поды задач сборщика. Ключ и прокси хранятся только в `.env`, который не коммитится. В `values.yaml` и в git их нет.
...
- **Под задачи Airflow падает сразу после старта.** Посмотрите `kubectl logs <pod> -n estate` Частая причина — неверный путь к репозиторию в VM. `make up` подставляет его сам. Если ставите чарт вручную, проверьте `global.projectHostPath`.
